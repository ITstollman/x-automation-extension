...
(function () {
  ...
  if (window.__xliftExecutorMounted) return;
  window.__xliftExecutorMounted = true;

  const LOG = (...args) => console.log("[Xlift executor]", ...args);
  ...
  async function postAction({ text }) {
    if (!text) throw new Error("text is required");
    LOG("post tweet, len=" + text.length);

    // Open the compose dialog via the side-nav button when on x.com,
    // otherwise navigate to /compose/post directly.
    const composeBtn = document.querySelector('[data-testid="SideNav_NewTweet_Button"]');
    if (composeBtn) {
      realClick(composeBtn);
    } else {
      await spaNav('/compose/post');
    }

    // Wait for the compose textarea — note we explicitly want the FIRST
    // composer (a reply has its target tweet visible above; new post does
    // not, but the same testid matches both).
    const editor = await waitFor('[data-testid="tweetTextarea_0"]', 6000, "compose-textarea");
    if (!editor) throw new Error("Compose dialog didn't open");

    // Reject when this turned out to be a reply composer (modal contains
    // a tweet article — Auto Posts must not target an existing tweet).
    const dialog = editor.closest('[role="dialog"], [aria-modal="true"]');
    const isReplyModal = dialog && dialog.querySelector('article[data-testid="tweet"]');
    if (isReplyModal) throw new Error("Compose dialog opened in REPLY mode — abort");
    ...
    const sendBtn = await waitFor(() => {
      ...
      const disabled = b.disabled || b.getAttribute("aria-disabled") === "true";
      return disabled ? null : b;
    }, 4000, "post-send-enabled");
    if (!sendBtn) throw new Error("Post button never enabled");
    realClick(sendBtn);
    ...
    const cleared = await waitFor(() => {
      const e = document.querySelector('[data-testid="tweetTextarea_0"]');
      return !e || (e.textContent || "").trim() === "" ? true : null;
    }, 6000, "post-editor-clear");
    if (!cleared) throw new Error("Post didn't go through");
  ...
  const HANDLERS = {
    ...
    "post": async (payload) => {
      try { return { ok: true, result: await postAction(payload) }; }
      catch (err) { return { ok: false, error: err?.message || String(err) }; }
    },
