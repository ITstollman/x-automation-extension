...
(function () {
  ...
  if (window.__xliftAccountMounted) return;
  window.__xliftAccountMounted = true;

  const LOG = (...args) => console.log("[Xlift account]", ...args);
  ...
  async function registerIfNeeded() {
    if (!window.xliftBackend) return; // lib/api.js not loaded
    if (!(await window.xliftBackend.isConnected())) return; // user hasn't pasted a key yet
    ...
    try {
      const res = await window.xliftBackend.connectAccount({
        handle,
        displayName: scrapeDisplayName(),
        avatar: scrapeAvatar(),
      });
      LOG("backend returned accountId:", res.accountId);
      await window.xliftBackend.setActiveAccount(res.accountId, handle);
      lastReportedHandle = handle;
    } catch (err) {
      console.error("[Xlift account] registration failed:", err.message);
    }
  }
